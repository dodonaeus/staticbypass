...
class junkpile:
    ...
    def chunk_and_obfuscate(self, encoded_script, debug=False):
        chunks = []
        ...
        while i < len(assignments):
            ...
            for _ in range(3): grouped_lines.append(self.generate_dead_code()) # modify range number to apply more/less dead code between generated strings
            i += group_size
    ...
    def generate_execution_line_array_style(self, array_var, debug=False):
        def split_str(s):
            parts = []
            i = 0
            while i < len(s):
        ...
        final_var = self.generate_var()
        trailing_var = self.generate_var()
    ...
    def wrap_final_string(self, var_names, debug=False):
        chunk_array_var = self.generate_var()  # random fake cmdlet-style var like ${Get-Help -Verbose -Path .\Temp && ($xyrt)}
        ...
        while i < len(var_names):
            ...
            for _ in range(3): lines.append(self.generate_dead_code()) # modify range number to apply more/less dead code between generated strings
            i += group_size

        # Execution line using array join
        lines.append(self.generate_execution_line_array_style(chunk_array_var, debug))

        return "\n".join(lines)


    # main obfuscation function for base64 handling
    def obfuscate_script(self, input_code, debug=False):
        stripped_code = self.strip_comments(input_code)
        ...
        lines, vars_used, chunks = self.chunk_and_obfuscate(encoded, debug=debug)
        final_line = self.wrap_final_string(vars_used, debug=debug)
    ...
    def apply(self, filepath):
        ...
        try:
            obfuscated = self.obfuscate_script(content)
